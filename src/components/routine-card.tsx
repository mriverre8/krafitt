...
export function RoutineCard(props: RoutineCardProps) {
    ...
    return (
        <li
            className={`${cardLinkClass} ${props.isActive ? 'border-blaze' : ''}`}
        >
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                    <Link
                        href={`/routines/${props.id}`}
                        className="display hover:text-blaze text-2xl"
                    >
                        {props.name}
                    </Link>
                ...
                {props.isActive ? (
                    <span className="bg-blaze text-on-accent flex shrink-0 items-center gap-1 rounded-full px-3 py-1 text-sm font-semibold">
                        <Flame
                            size={13}
                            aria-hidden
                        />
                    ...
                    <ActionButton
                        ...
                        className="lift border-line text-muted hover:border-blaze hover:text-blaze shrink-0 rounded-full border px-3 py-1 text-sm font-semibold"
                    >
