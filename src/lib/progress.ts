...
export type Logs = Record<string, Record<number, SetValue | undefined> | undefined>;

export type FlatSet = { exerciseId: string; exerciseIndex: number; setIndex: number };
...
export function isSetFilled(logs: Logs, exerciseId: string, setIndex: number): boolean {
...
export function isSetEnabled(
  ...
  const i = flat.findIndex((s) => s.exerciseId === exerciseId && s.setIndex === setIndex);
  if (i < 0) return false;
  return flat.slice(0, i).every((s) => isSetFilled(logs, s.exerciseId, s.setIndex));
}

/** The workout is done once the last set is filled in. */
export function isSessionComplete(exercises: ExercisePlan[], logs: Logs): boolean {
  const flat = flatSets(exercises);
  return flat.length > 0 && flat.every((s) => isSetFilled(logs, s.exerciseId, s.setIndex));
}
